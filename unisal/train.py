...
class Trainer(utils.KwConfigClass):
    ...
    def __init__(
        ...
        hollywood_cfg=None,
        ucfsports_cfg=None,
        final_test_mit1003_cfg=None,
        final_test_mit300_cfg=None,
        ...
        if debug:
            ...
            if self.suffix.endswith("_debug"):
                self.suffix += "_debug"

            self.data_cfg.update({"subset": 0.1})
            self.salicon_cfg.update({"subset": 0.2})
            self.hollywood_cfg.update({"subset": 0.04})
            self.ucfsports_cfg.update({"subset": 0.8})
    ...
    def _save_comparison_image(self, output_path, img, sal_map, fix_map, pred_map, scores, source, sample_id):
        """Save a comparison image with original, ground truth, and prediction"""
        import cv2
        import numpy as np
        ...
        sal_map_norm = normalize_map(sal_map)
        fix_map_norm = normalize_map(fix_map)
        pred_map_norm = normalize_map(pred_map)
        ...
        sal_heat = to_heatmap(sal_map_norm)
        fix_heat = to_heatmap(fix_map_norm)
        pred_heat = to_heatmap(pred_map_norm)
        
        # Overlay on original image
        def overlay(img, heat, alpha=0.5):
            return cv2.addWeighted(img, 1-alpha, heat, alpha, 0)
        
        img_bgr = img[:, :, ::-1] if len(img.shape) == 3 and img.shape[2] == 3 else cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        sal_overlay = overlay(img_bgr, sal_heat) if sal_map_norm is not None else img_bgr
        fix_overlay = overlay(img_bgr, fix_heat) if fix_map_norm is not None else img_bgr
        pred_overlay = overlay(img_bgr, pred_heat)
        
        # Create canvas
        canvas_h = h + 60  # Extra space for text
        canvas_w = w * 4 + pad * 3
        canvas = np.ones((canvas_h, canvas_w, 3), dtype=np.uint8) * 255
        
        # Place images
        canvas[30:h+30, 0:w] = img_bgr
        canvas[30:h+30, w+pad:2*w+pad] = sal_overlay
        canvas[30:h+30, 2*w+2*pad:3*w+2*pad] = fix_overlay
        canvas[30:h+30, 3*w+3*pad:4*w+3*pad] = pred_overlay
        ...
        cv2.putText(canvas, "Original", (10, 20), font, font_scale, color, thickness)
        cv2.putText(canvas, "GT Saliency", (w+pad+10, 20), font, font_scale, color, thickness)
        cv2.putText(canvas, "GT Fixation", (2*w+2*pad+10, 20), font, font_scale, color, thickness)
        cv2.putText(canvas, "Prediction", (3*w+3*pad+10, 20), font, font_scale, color, thickness)
        
        # Add scores if available
        if scores is not None:
            score_text = f"KLD:{scores[0]:.3f} NSS:{scores[1]:.3f} CC:{scores[2]:.3f}"
            cv2.putText(canvas, score_text, (10, h+50), font, 0.5, color, 1)
    ...
    def save_loss_plots(
            self,
            out_dir=None,
            make_one_figure_per_dataset=True,
            dpi=200,
    ):
        """
        Save loss curve plots from self.all_scalars.

        What you get:
          - For each dataset (conv/salicon/ucfsports/...):
              A figure with subplots:
                  loss (total), loss_0, loss_1, loss_2, ...
              Each subplot overlays train & valid curves.

        Args:
          out_dir: output directory (default: self.train_dir / "plots")
          make_one_figure_per_dataset: if True -> separate PNG per dataset
                                      if False -> one big figure for all datasets
          dpi: saved image dpi
        """
        import math
        ...
        def _extract_series(src_dict, key, phase):
            """
            src_dict: self.all_scalars[src]
            key: "loss" or "loss_0" ...
            phase: "train" or "valid"
            Returns (epochs, values) sorted by epoch.
            """
            if key not in src_dict:
                return None
            if phase not in src_dict[key]:
                return None
            epoch_to_val = src_dict[key][phase]
            if not isinstance(epoch_to_val, dict) or len(epoch_to_val) == 0:
                return None

            epochs = sorted(epoch_to_val.keys())
            vals = [epoch_to_val[e] for e in epochs]
            # Filter out NaNs/None while keeping alignment
            clean = [(e, v) for e, v in zip(epochs, vals) if _is_num(v)]
            if not clean:
                return None
            epochs, vals = zip(*clean)
            return list(epochs), list(vals)

        def _list_loss_keys(src_dict):
            """
            Find all keys like: loss, loss_0, loss_1, ...
            """
            keys = []
            ...
            # Put total loss first, then components in numeric order
            def sort_key(k):
                if k == "loss":
                    return (-1, -1)
                # loss_0 -> 0
        ...
        def _plot_dataset(src, src_dict, out_file):
            loss_keys = _list_loss_keys(src_dict)
            if not loss_keys:
                return
            ...
            for i, key in enumerate(loss_keys):
                ax = axes[i]
                # Train + valid on same axes
                for phase in ("train", "valid"):
                    series = _extract_series(src_dict, key, phase)
                    if series is None:
                        continue
                    epochs, vals = series
                    ax.plot(epochs, vals, marker="o", linewidth=1.5, label=phase)

                ax.set_title(f"{src} / {key}")
                ax.set_xlabel("epoch")
                ax.set_ylabel("value")
        ...
        # -------------------------
        if out_dir is None:
            out_dir = self.train_dir / "plots"
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)

        # Filter only dataset-like entries
        datasets = []
        for src, src_dict in self.all_scalars.items():
            if isinstance(src_dict, dict) and any(k == "loss" or str(k).startswith("loss_") for k in src_dict.keys()):
                datasets.append((src, src_dict))

        if not datasets:
            print("No loss keys found in self.all_scalars.")
            return

        if make_one_figure_per_dataset:
            for src, src_dict in datasets:
                out_file = out_dir / f"{src}_loss_curves.png"
                _plot_dataset(src, src_dict, out_file)
        else:
            # One big figure: each dataset gets its own row; columns are loss keys union
            all_keys = set()
            for _, src_dict in datasets:
                all_keys.update(_list_loss_keys(src_dict))
            all_keys = sorted(
                list(all_keys),
                key=lambda k: (-1, -1) if k == "loss" else (0, int(k.split("_")[1]) if k.startswith("loss_") and
                                                                                       k.split("_")[
                                                                                           1].isdigit() else 999)
            )
            ...
            for r, (src, src_dict) in enumerate(datasets):
                for c, key in enumerate(all_keys):
                    ...
                    for phase in ("train", "valid"):
                        ...
                        if series is None:
                            continue
                        epochs, vals = series
                        ax.plot(epochs, vals, marker="o", linewidth=1.2, label=phase)
                    ax.set_title(f"{src}/{key}")
                    ax.grid(True, alpha=0.25)
                    if r == nrows - 1:
                        ax.set_xlabel("epoch")
            ...
            fig.tight_layout()
            out_file = out_dir / "all_datasets_loss_curves.png"
            fig.savefig(str(out_file), dpi=dpi)
            plt.close(fig)
            print(f"Saved: {out_file}")
